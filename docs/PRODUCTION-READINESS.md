...
| Date | Closed | Commits |
|---|---|---|
| 2026-09-07 | SEC-1, SEC-2, SEC-7 — file routes and exports authorize from the live user; invite links gated to admins and invite/consent emails marked sensitive; the two non-action helpers moved out of `"use server"` files | `effcb27` |
| 2026-09-07 | MONEY-1, MONEY-7, MONEY-11 — late notice fires the day grace lapses (`shouldSendLateNotice`, unit-tested); nightly cron re-runs reconciliation; a redelivered `checkout.session.completed` can no longer regress a settled payment | `83569a1` |
| 2026-09-07 | DATA-1, DATA-3 — property delete blocks on any lease history; Plaid removals mark payments FAILED and keep the lease match | `05d6af3` |
| 2026-09-07 | SEC-4, SEC-5 — CSV formula neutralisation; `/api/report-error` requires same-origin, is rate-limited per IP, and dedupes repeats | `38a8a17` |
| 2026-09-07 | OPS-5, OPS-9 — `e2e:documents` and `e2e:portfolio-import` run in CI against the landlord10 seed; MAINTAINER §8/§13 and the e2e README brought up to date. Wiring the importer suite in immediately paid for itself: the SEC-7 move of `loadExistingPortfolio` into `src/lib/portfolio-import.ts` had pulled Prisma into the mapping form's client bundle and broken the import page (typecheck, lint and unit tests all stayed green). Now in its own server-only module, `src/lib/portfolio-import-data.ts`. | this commit |

Items below keep their original finding text so the reasoning stays on record;
closed ones are marked ✅ in the table.
...
1. **Two HIGH security findings and six money-correctness bugs** at the
   ...
   confirmed. *(All fixed 2026-09-07 — see Progress.)*
...
| MONEY-7 | High | ✅ **Done** (83569a1) — **Nightly cron never re-runs reconciliation.** Only the manual "Run rent" button does. `SHORT` is time-dependent and only recomputed on write events, so a partial payment shows "Matched" indefinitely after grace lapses. **Fix:** call `applyReconciliationForOrganization` in the cron. | Claude | S |
...
| MONEY-11 | Medium | ✅ **Done** (83569a1) — Stripe webhook can regress a settled payment: `onCheckoutCompleted` has no "already SUCCEEDED" guard (unlike `onIntentSucceeded`), so a redelivered `checkout.session.completed` sets `PROCESSING, paidAt: null` and sends a second receipt. **Fix:** never downgrade; skip when SUCCEEDED/REFUNDED. | Claude | S |

### Data loss

| ID | Sev | Finding | Owner | Effort |
|---|---|---|---|---|
| DATA-1 | High | ✅ **Done** (05d6af3) — **Deleting a property destroys ended-lease history and orphans payments.** Guard only blocks *active* leases; Unit→Lease→Charge cascade, Payment.leaseId set-null. Inconsistent with `deleteUnitAction`, which blocks on any lease. One STAFF click erases a building's billing history. **Fix:** block on any lease or payment; offer archive. | Claude | S |
...
| DATA-3 | High | ✅ **Done** (05d6af3) — **Plaid "removed" transactions hard-delete payments a human matched.** `deleteMany` on removed ids, including ones with a manual lease match; some banks reissue ids on pending→posted. **Fix:** mark FAILED "Removed by bank", keep `leaseId`. | Claude | S |
...
| OPS-5 | Medium | ✅ **Done** (this commit) — Two feature e2e suites are not in CI: `e2e:documents` and `e2e:portfolio-import` (need the landlord10 seed + `DEMO_PAYMENTS` in the job). The vault and importer have zero automated regression protection. | Claude | S |
...
| OPS-9 | Low | ✅ **Done** (this commit) — Doc drift: MAINTAINER §13 still says photos are D1 blobs and `ERROR_ALERT_EMAIL` is unset (both resolved); e2e README says eleven suites (fifteen). | Claude | S |
...
1. ~~**This week (Claude):** SEC-1, SEC-2, MONEY-1, MONEY-7, DATA-1, DATA-3, SEC-4, SEC-5, MONEY-11, then SEC-7, OPS-5, OPS-9.~~ **Done 2026-09-07** — see Progress above.
